    ...
    <app-dashboard-layout
      ...
      <mat-tab-group #tabGroup animationDuration="0ms">
        ...
        <mat-tab label="View Links">
          <section class="card" style="margin-top:20px;">
            <div class="card-header">
              <div class="card-title">Global Links Data</div>
              <p class="card-subtitle">Create and manage the top-level navigation groups.</p>
            </div>
            ...
            <div class="card-body table-wrap">
              @if (links.length > 0) {
                ...
                <div class="empty-state">
                  <span class="material-symbols-outlined empty-icon">public</span>
        ...
        <mat-tab [label]="editingLink ? 'Edit Link' : 'Add Link'">
            <section class="card" style="margin-top:20px;">
              <div class="card-header">
                <div class="card-title">{{ editingLink ? 'Edit Global Link' : 'Add Global Link' }}</div>
              </div>
              <div class="card-body">
                ...
                <input class="form-control" formControlName="displayName" placeholder="e.g. Administration" />
...
export class GlobalLinksComponent implements OnInit {
  ...
  activeSection = 'global-links';
  navGroups = ADMIN_NAV_GROUPS;
  links: GlobalLink[] = [];
  editingLink: GlobalLink | null = null;

  showConfirm = false;
  confirmMessage = '';
  deletingLink: GlobalLink | null = null;
  saving = false;
  ...
  loadLinks(): void {
    this.api.getGlobalLinks().subscribe({
      ...
      error: () => this.toast.error('Failed to load global links.')
    });
  }

  openForm(link?: GlobalLink): void {
  ...
  saveLink(): void {
    ...
    const request = this.editingLink
      ? this.api.updateGlobalLink(this.editingLink.id, payload)
      : this.api.createGlobalLink(payload);

    request.subscribe({
      next: () => {
        this.toast.success('Global link saved.');
        this.editingLink = null;
        this.loadLinks();
        this.refreshMenu();
        this.saving = false;
      ...
      error: err => {
        this.toast.error(err.error?.message || 'Failed to save global link.');
        this.saving = false;
      }
    });
  }

  confirmDeactivate(link: GlobalLink): void {
    if (!link.active) return;
    this.deletingLink = link;
    this.confirmMessage = `Deactivate global link "${link.displayName}"? This will also deactivate any linked primary links.`;
  ...
  deactivateLink(): void {
    ...
    this.api.deleteGlobalLink(this.deletingLink.id).subscribe({
      next: () => {
        this.toast.success('Global link deactivated.');
        this.showConfirm = false;
        this.deletingLink = null;
        this.loadLinks();
        this.refreshMenu();
      ...
      error: () => {
        this.toast.error('Failed to deactivate global link.');
        this.showConfirm = false;
      }
    });
  }
  ...
  moveOrder(index: number, delta: number): void {
    ...
    this.api.reorderGlobalLinks(payload).subscribe({
      next: () => {
        this.loadLinks();
        this.refreshMenu();
      },
